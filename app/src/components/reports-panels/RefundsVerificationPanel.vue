<template>
    <div class="panel">
        <h3>Refunds Verification Report</h3>
        <p>
            Select a date range or enter order IDs to generate a CSV that cross-checks Woo refunds, Braintree credits,
            and Complyt refund transactions.
        </p>
        ...
        <n-modal v-model:show="showModal" preset="dialog" title="Report Ready">
            <template #default> Your Refunds Verification Report is ready for download. </template>
            <template #action>
                <a
                    :href="downloadUrl"
...
// same props/emit API as the other panels
...
// IMPORTANT: point to the new backend report_type
const {
    loading,
    ...
    downloadUrl,
    runReport,
} = useReportChunks("refunds-verification", 30);
